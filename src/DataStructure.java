...
public class DataStructure implements DT {
	...
	public Point[] nearestPair() {
		...
		if(n == 2){
			if(axis){
				return new Point[] {xAxis.getHead().getData(), xAxis.getTail().getData()};
			}
			return new Point[] {yAxis.getHead().getData(), yAxis.getTail().getData()};
		}
		else if(n<=1){
			return new Point[0];
		}
		...
		double d = dist(closest[0], closest[1]);
		Point[] closestPairInStrip = nearestPairInStrip(getMedian(axis), 2*d, axis);
		return calcMinDistance(closestPairInStrip, closest);
	}
